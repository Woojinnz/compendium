...
# Load the new comprehensive model
model = joblib.load("model_pos.pkl")
scaler = joblib.load("scaler_pos.pkl")
...
def plot_3d_trajectory(df, file_id, title_suffix=""):
    ...
    ax.set_title(f'{title_suffix} 3D Trajectory')
...
def plot_processed_coordinates(processed_df, file_id, start, end):
...
# Main processing loop - updated to use comprehensive features
for i in range(1, 2):
    try:
        file_path = f"trace_2.txt"
        ...
        # Use comprehensive feature classification
        ...
        # Plot results
        plot_3d_trajectory(pos_data, i, "Original")
        plot_3d_trajectory(processed_data, i, "Smoothed")
        plot_processed_coordinates(processed_data, i, start, end)
